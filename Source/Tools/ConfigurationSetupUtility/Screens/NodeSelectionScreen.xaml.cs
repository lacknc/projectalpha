...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class NodeSelectionScreen : UserControl, IScreen
    {
        ...
        private void UpdateDataGrid()
        {
            ...
            {
                m_infoTextBlock.Text = "Please select the node you would like the Palpha to use.";
            }
            else
            {
            ...
            if (applyChangesToService)
                nodeId = GetNodeIdFromConfigFile("Palpha.exe.config");
            else if (applyChangesToLocalManager)
                nodeId = GetNodeIdFromConfigFile("PalphaManager.exe.config");
            else
                nodeId = Guid.Empty;
            ...
            if (applyChangesToService)
                TryAddNodeInfo("Palpha.exe.config");

            if (applyChangesToLocalManager)
                TryAddNodeInfo("PalphaManager.exe.config");
        ...
        private IDbConnection GetConnectionFromConfigFile()
        {
            ...
            string configFileName = Directory.GetCurrentDirectory() + "\\Palpha.exe.config";
        ...
        {
            return GetValueOfSystemSetting("Palpha.exe.config", "CompanyName");
        }
        ...
        // Occurs when the user selects a different node to be used by the Palpha.
